...
## Edit config files:
```bash
vim configs/apis/config_readypy_yaml.yaml
vim configs/apis/config_readypy_template.yaml
```

## Create data paths based on config file
```bash
mkdir -p ~/datasets/ready/ready_py/recordings && cd ~/datasets/ready/ready_py/recordings && mkdir -p test
```
...
vim -O scripts/apis/ready_py.bash configs/apis/config_readypy_template.yaml src/ready/apis/holoscan/ready/python/ready.py
##Ctrl+WW to swap windows; :vertical resize 100
```

#TODO 
* [ ] add model,path,name to config files and refactor script to pass such values to avoid long arguments inputs!
* [ ] 2/n adds recording and replayer
* [ ] 3/n remove and udpates this list of commands
* [ ]The following section is confusing. Let's clean it. Models were tested but need to be cleaned or perhaps just remove them!
```
## novel
