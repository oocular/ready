...
ffmpeg -framerate ${NNframes} -pattern_type glob -i "${NNframes}_duplicated_frames_of_${FRAMEFILENAMEID}/*."${IMAGE_EXTENSION} -c:v libx264 video_${NNframes}_duplicated_frames_of_${FRAMEFILENAMEID}_channels${CHANNELS}.mp4
ffmpeg -i video_${NNframes}_duplicated_frames_of_${FRAMEFILENAMEID}_channels${CHANNELS}.mp4  #check video
...
ffmpeg -i video_${NNframes}_duplicated_frames_of_${FRAMEFILENAMEID}_channels${CHANNELS}.mp4 -pix_fmt rgb24 -f rawvideo pipe:1 | python ../../../src/ready/apis/holoscan/utils/convert_video_to_gxf_entities.py --width ${WIDTH} --height ${HEIGHT} --channels ${CHANNELS} --framerate ${NNframes} --basename video_${NNframes}_duplicated_frames_of_${FRAMEFILENAMEID}_channels${CHANNELS}
